...
pub struct CommodityMaker {
    ...
    spread: f64,
    price_queue: Mutex<VecDeque<CpPrice>>,
...
impl CommodityMaker {
    ...
    pub fn run(&self) {
        ...
        let price = CpPrice::new(self.symbol.to_string(), price_id, mid, mid - self.spread, mid + self.spread, Local::now());
        {
            ...
            self.client.lock().unwrap().send(serde_json::to_string(&price).unwrap());
            price_queue.push_back(price);
        }
    }

    pub fn ref_latest_price(&self) -> Option<CpPrice> {
